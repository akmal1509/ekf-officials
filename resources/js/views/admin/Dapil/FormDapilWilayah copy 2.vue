        ...
        <form @submit.prevent="handleSubmit" class="mt-4 space-y-4">
            ...
            <div class="">
                <Select label="Provinsi" text="Pilih Provinsi" :data="initProvince" v-model="form.provinceId"
                    :search="form.provinceId" @update:search="form.provinceId = $event"></Select>
                <!-- {{ dapilCategories }} -->
            </div>
            <div class="">
                <Select label="Kota / Kabupaten" text="Pilih Kota" :data="initCity" v-model="form.CityId"
                    :search="selectedCity" @update:search="form.cityId = $event"></Select>
                <!-- {{ dapilCategories }} -->
            </div>
            <div class="">
    ...
    setup(props) {
        ...
        const fetchEdit = async () => {
            ...
            if (dapilWils) {
                ...
                form.value = dapilWils.value.data
                provinceId = form.value.provinceId
                selectedCity = form.value.cityId
        ...
        watch(
            ...
            (newId, oldId) => {
                if (newId !== provinceId) {
                    selectedCity = 0
                    console.log('province berubah')
                }
                console.log('ini selected City:' + selectedCity)
                fetchCity(newId);
            }
        );
        watch(
